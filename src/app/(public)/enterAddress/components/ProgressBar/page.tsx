...
const ProgressBar: React.FC<ProgressBarProps> = ({ progress }) => {
	...
	const segments = Array.from({ length: totalSegments }, (_, i) => i < filledSegments);
				...
				{segments.map((isFilled, index) => (
					<div
						...
						className={`w-full max-w-[48.75px] h-[8px] ${
							isFilled ? 'bg-primary-normal' : 'bg-label-disable'
						} ${index === 0 ? 'rounded-l-full' : ''} ${index === totalSegments - 1 ? 'rounded-r-full' : ''}`}
					></div>
				))}
			</div>
		</div>
